...
buttonGrafS = Button(root,text="Salida",font=("Quicksand",12),command=lambda:graficaSalida(calculaConv(txtXn.get(),txtHn.get()),varCir.get()))

buttonGrafE = Button(root,text="Entrada",font=("Quicksand",12),command=lambda:graficaIn(txtXn.get(),txtHn.get(),varCir.get()))
...
buttonGrafE.place(x=320,y=300)llection = True)
...
def graficaSalida(dtS, auxG):
	if auxG == 0:
		...
		plt.figure('Grafica Salida')
		plt.stem(dSx,dyP, use_line_collection = True)
		plt.xlabel('x')
		plt.ylabel('y')
		plt.title('Xn * Hn')
		plt.show()
